...
import 'package:subgkart/components/BNavigation.dart';
import 'package:subgkart/fliter_products.dart';
import 'package:subgkart/components/category_view.dart';
import 'package:subgkart/components/home_products.dart';
import 'package:subgkart/profile.dart';
import 'package:subgkart/search.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(key: _scaffoldKey,backgroundColor: Colors.white10.withOpacity(0.95),drawer: buildContainer(context,  url),appBar: AppBar(
        ...
        builder: (context, snapshot) {
          return GestureDetector(
            child: Column(
              ...
              children: [
                Text('Delivery Address',style: GoogleFonts.poppins(fontWeight: FontWeight.normal, color: Colors.white,fontSize: 12),
                ),
      ...
      body: ListView(shrinkWrap: true,
        ...
        children: [
                     ...
                     return Padding(
                       ...
                       child: Container(width: MediaQuery.of(context).size.width,
                         child: GridView.builder(
                           shrinkWrap: true,itemCount: snapshot.data.documents.length,
                           physics: BouncingScrollPhysics(),
                           gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                             crossAxisCount: 3,  childAspectRatio: 3 / 3.3,
                           ),
                           itemBuilder: (BuildContext context, int index) {
                             return CategoryView(snapshot: snapshot.data.documents[index],onClick: (){
          ...
          StreamBuilder<QuerySnapshot>(
              stream: dataProvider.category(),
              builder: (context, snapshot) {
                return ListView.builder(shrinkWrap: true,itemCount: snapshot.data.documents.length,
                  physics: BouncingScrollPhysics(),
                  itemBuilder: (BuildContext context, int index) {
                    return HomeProducts(snapshot: snapshot.data.documents[index],);
                  },);
              }
          ),
          SizedBox(height: 20)
      ...
      child: ListView(
        children: [
            ...
            child: Container(width: 110, height: 110,
              ...
              child: Neumorphic(
                ...
                child: ClipRRect(
                  ...
                  child: UserProvider.profile!= null && url.isNotEmpty?CachedNetworkImage(
          ...
          Padding(padding: const EdgeInsets.all(8.0),
            child: Center(
                child: Text(UserProvider.name??'Guest', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 18,color: Colors.black),
                )),
          ),
          Padding(
            padding: const EdgeInsets.only(left: 8, right: 8, bottom: 10),
            child: Center(
                child: Text(UserProvider.user.phoneNumber.toString()??'+91 xxxxxxxx',
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              Center(child: Icon(Icons.info_outline,size: 65,color: Colors.redAccent,),),
              SizedBox(height: 25),
              Padding(
                padding: const EdgeInsets.only(left: 8,right: 8),
                child: Text('Are sure you want to logout?',style:GoogleFonts.poppins(color: Colors.black),textAlign: TextAlign.center,),
              ),
              SizedBox(height: 25),
